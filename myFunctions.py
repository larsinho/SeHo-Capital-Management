...
def efficient_return(mean_returns, cov_matrix, target):
    ...
    constraints = ({'type': 'eq', 'fun': lambda x: portfolio_volatility(x) - target},{'type': 'eq', 'fun': lambda x: np.sum(x) - 1})
    bounds = tuple((0,1) for asset in range(num_assets))
    initialVal = num_assets*[1./num_assets,]
    result = sco.minimize(portfolio_return, initialVal, method='SLSQP', bounds=bounds, constraints=constraints)
    return result
...
def display_ef_with_selected(table, mean_returns, cov_matrix, risk_free_rate):
    ...
    efficient_portfolios = efficient_frontier(mean_returns, cov_matrix, targets)
    returns_efficient = [portfolio_annualised_performance(p['x'], mean_returns, cov_matrix)[1] for p in efficient_portfolios]
    ax.plot(targets, returns_efficient, linestyle='-.', color='black', label='efficient frontier')
...
if __name__ == "__main__":
    ...
    table.columns =  ['Bonds', 'Agriculture', 'Emerging', 'Europe', 'Japan', 'US', 'Gold', 'Oil'] #table.columns = [col[1] for col in table.columns]
    table = table.dropna()
